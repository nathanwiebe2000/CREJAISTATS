...
%DIF DEL CRPE7-old.tex   Thu Jul 30 18:08:56 2015
%DIF ADD CRPE7.tex       Sun Aug  2 21:51:46 2015
%DIF 2a2-4
%============================================================================= %DIF > 
% BEGIN UNFORGIVABLE HACKS %DIF > 
%============================================================================= %DIF > 
%DIF -------

%DIF 3a6-21
\makeatletter %DIF > 
\def\@bibdataout@aps{% %DIF > 
 \immediate\write\@bibdataout{% %DIF > 
  @CONTROL{% %DIF > 
   apsrev41Control,author="08",editor="1",pages="0",title="0",year="1",eprint="1"% %DIF > 
  }% %DIF > 
 }% %DIF > 
 \if@filesw %DIF > 
  \immediate\write\@auxout{\string\citation{apsrev41Control}}% %DIF > 
 \fi %DIF > 
}% %DIF > 
\makeatother % Phew. %DIF > 
 %DIF > 
%============================================================================= %DIF > 
% END UNFORGIVABLE HACKS %DIF > 
%============================================================================= %DIF > 
%DIF -------
...
%DIF 106a125-128
 %DIF > 
\hyphenation{FPGA} %DIF > 
\hyphenation{FPGAs} %DIF > 
\hyphenation{RFPE} %DIF > 
%DIF -------
...
\begin{abstract}
    ...
    estimation that does not require that the user to infer the bits of the
    eigenphase \DIFdelbegin \DIFdel{from highest- to lowest-order}\DIFdelend \DIFaddbegin \DIFadd{in reverse order}\DIFaddend ; rather it directly infers the phase and
    estimates the uncertainty in the phase direcly from experimental data. Our
    method is highly flexible, \DIFaddbegin \DIFadd{recovers from failures, and }\DIFaddend can be run in the
    presence of substantial decoherence and other experimental imperfections
    and is as fast or faster than existing algorithms.  The simplicity and low
    memory footprint of our method also make it amenable to embedded  or FPGA
    applications.
\end{abstract}
...
the methodologies used for \DIFdelbegin \DIFdel{the inference methods }\DIFdelend \DIFaddbegin \DIFadd{inference }\DIFaddend stretch back to the nineties,
...
to \DIFdelbegin \DIFdel{the phase estimationproblems. This }\DIFdelend \DIFaddbegin \DIFadd{phase estimation. Our }\DIFaddend method not only provides performance
advantages over existing \DIFdelbegin \DIFdel{ITPE }\DIFdelend \DIFaddbegin \DIFadd{iterative }\DIFaddend methods~\cite{Kit96,kitaev2002classical}, but
\DIFdelbegin \DIFdel{also is }\DIFdelend \DIFaddbegin \DIFadd{is also }\DIFaddend robust to experimental imperfections such as depolarizing noise and
...
sampling can be used to efficiently approximate Bayesian inference\DIFaddbegin \DIFadd{, }\DIFaddend thereby
allowing PE to inherit the speed and robustness of Bayesian approaches while
retaining the efficiency of traditional \DIFdelbegin \DIFdel{PE }\DIFdelend methods.

%DIF < Phase estimation (PE) is a technique of fundamental importance to quantum information.  The purpose of the algorithm is to estimate the eigenphase corresponding to an input eigenstate of a unitary $U$ within a user specified precision.  The protocol forms the core of quantum simulation algorithms, linear systems algorithms and the amplitude estimation algorithm to name a few applications~\cite{shor_polynomial-time_1995,BHM+02,ADL+05,harrow2009quantum,lanyon2010towards}.  As a result, methods that even modestly improve the time and space complexities of phase estimation or increase the algorithm's robustness to experimental errors are of great importance to both quantum computing and metrology~\cite{Kit96,kitaev2002classical,higgins2007entanglement,SHF14}.  Our work not only provide a fast PE algorithm but also a more general one that can tolerate substantial experimental imperfections.
...
function \DIFdelbegin \DIFdel{, }\DIFdelend $P(0|\phi;\theta,M)$\DIFdelbegin \DIFdel{, is easy to compute for }\DIFdelend \DIFaddbegin \DIFadd{. For }\DIFaddend phase estimation,
\begin{gather}
...
as credible regions, can also be easily extracted from the posterior
distribution~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{granade_robust_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{granade_robust_2012,ferrie_high_2014}
}%DIFAUXCMD
}\DIFaddend . These estimates of
the uncertainty are crucial for our method because they not only allow the
user to prematurely end the protocol when a fixed accuracy has been reached,
but also because they allow us to design highly informative experiments for
finding the true eigenvalue.
...
\DIFaddend Exact Bayesian inference is impossible in cases where the eigenphase is
continuous.   As a result\DIFaddbegin \DIFadd{, }\DIFaddend approximations to Bayesian inference are needed to
make the procedure tractable.  Rather than \DIFdelbegin \DIFdel{naively }\DIFdelend \DIFaddbegin \DIFadd{na\"ively }\DIFaddend discritizing the prior
distribution, modern methods \DIFdelbegin \DIFdel{use sampling methods to discretize }\DIFdelend \DIFaddbegin \DIFadd{discretize by sampling from }\DIFaddend the prior and
...
\cite{haykin2004kalman,smith2013sequential}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{haykin2004kalman,smith2013sequential,isard_condensationconditional_1998}
}%DIFAUXCMD
}\DIFaddend .
...
controller or an FPGA. \DIFdelbegin %DIFDELCMD < { %%%
\DIFdelend With the increasing use of FPGAs in the control of
quantum information experiments,
\cite{shulman_suppressing_2014,casagrande_design_2014,hornibrook_cryogenic_2015},
\DIFdelbegin \DIFdel{embedding inference algorithms
}\DIFdelend \DIFaddbegin \DIFadd{overcoming this limitation }\DIFaddend presents a
significant advantage to modern experiment design. \DIFdelbegin %DIFDELCMD < } %%%
\DIFdelend As a result, we propose
using a much simpler approach that we call \DIFdelbegin %DIFDELCMD < { %%%
\DIFdelend Rejection Filtering Phase
Estimation (RFPE)\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend . Rather than using a set of hypotheses that implicitly
...
Gaussian to model our initial prior, perform a Bayesian update on samples
drawn from the distribution and then refit the updated samples to a Gaussian.
This strategy is used in a number of particle filter approximations to the
posterior such as the extended Kalman filter and assumed density
filtering~\cite{haykin2004kalman,opper1998bayesian}.  We then further optimize
this method by approximating the Bayesian inference step using rejection
sampling.  This can reduce the memory required by a factor of $1~000$ or more\DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{,
as we need only consider a single sample at a time. Our }\DIFaddend algorithm is described
below and pseudocode is given in \DIFdelbegin \DIFdel{the appendix.
}\DIFdelend \DIFaddbegin \app{pseudocode}\DIFadd{.
}
...
\DIFaddend \begin{figure}[t!]
    ...
    \caption{\label{fig:ITPEcmp}
     Comparison of \DIFdelbeginFL %DIFDELCMD < { %%%
\DIFdelendFL RFPE \DIFdelbeginFL %DIFDELCMD < } %%%
\DIFdelendFL to ITPE for $t=10~000$ with $100$ samples for $\phi_{\rm true} = 2\pi k/t$ at each measurement.  
    }
\end{figure}
...
Although it is difficult to concretely predict the value of $m$ needed to make the error in the inference small, we show in \DIFdelbegin \DIFdel{the appendix }\DIFdelend \DIFaddbegin \app{stability} \DIFaddend that $m$ must scale at least as the inverse square of the relative fluctuations in the likelihood function.  Similarly, Markov's inequality shows that $m$ must scale at least as $\kappa_E/\int P(E|\phi;\theta,M)P(\phi) \mathrm{d}\phi$ to ensure that, with high probability, the number of accepted samples will be large enough to accurately estimate the mean.  We introduce $\kappa_E$ to compensate for small expected likelihoods.

%DIF < A further advantage of { RFPE} is that only a few hundred bits of memory are required, even if $m\gg 1$, because $\mu$ and $\sigma$ can be computed incrementally.  This not only allows PE to be run as a streaming algorithm but also enables execution in memory restricted environments such as embedded controllers or FPGAs.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The main issue that remains is how to optimally choose the parameters $\theta$
...
at a rate of tens of megahertz or faster.  Fortunately, there is an expedient
guess heuristic, known as the particle guess heuristic (PGH) that can give a
\DIFdelbegin \DIFdel{near optimal }\DIFdelend \DIFaddbegin \DIFadd{near-optimal }\DIFaddend experiment for this class of likelihood
functions~\cite{wiebe_hamiltonian_2014}\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \begin{align}
    M &= \left\lceil\frac{1.25}{\sigma}\right\rceil,~
    \theta \sim P(\phi).\label{eq:PGH}
\end{align}
The factor of $1.25$ comes from optimizing the cost of \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE}%DIFDELCMD < }%%%
\DIFdel{.   Non integer }\DIFdelend \DIFaddbegin \DIFadd{RFPE.   Non-integer }\DIFaddend $M$
are appropriate if \DIFdelbegin \DIFdel{$U=e^{-i H M}$}\DIFdelend \DIFaddbegin \DIFadd{$U=\ee^{-i H M}$}\DIFaddend .

%DIF < A small technical complication emerges because the likelihood function possesses branch cuts.  For example, the sample variance from the Gaussian $\mathcal{N}(0,1)$ will with high probability be much greater than that from $\mathcal{N}(\pi,1)$ if the results are folded into the interval $[0,2\pi]$.  This can be addressed by shifting the interval to remove this illusionary source of variance.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < $M$ does not need to be an integer in general.  In quantum simulation experiments where $U=e^{-iHt}$, fractional powers of the unitary can be executed %without incurring additional quantum cost.  It can be useful to ignore the ceiling function in~\eq{PGH}.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \fig{PEerror} shows the error incurred using \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE}\DIFaddend .  The most obvious feature is that the error shrinks exponentially with the number of experiments (which is proportional to the evolution time under the PGH) for $m>100$.  Roughly $150$ experiments are needed for the method to provide $32$ bits of accuracy in the median case.
We discuss the scaling in the mean in \DIFdelbegin \DIFdel{the appendix}\DIFdelend \DIFaddbegin \app{var-reduction}\DIFaddend .

The number of experiments needed to reach error $\epsilon$ scales as $O(\log(1/\epsilon))$ rather than $O(\log(1/\epsilon)\log\log(1/\epsilon))$ in Kitaev's method for iterative phase estimation~\cite{Kit96,kitaev2002classical}.  Concretely, after $150$ experiments the median error for Kitaev's PE algorithm (with $s=10$) is roughly $10$ million times that of RFPE.

Although the number of experiments needed is small, the experimental time need
not be.  If the error shrinks as \DIFdelbegin \DIFdel{$\epsilon\in \Theta(e^{-\lambda N})$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon\in \Theta(\ee^{-\lambda N})$ }\DIFaddend where
$N$ is the experiment number then \DIFdelbegin \DIFdel{$T_{\exp}\in O(\sum_{N=1}^{N_{\rm max}}e^{\lambda N})\in O(e^{\lambda N_{\rm max}})\in O(1/\epsilon)$}\DIFdelend \DIFaddbegin \DIFadd{$T_{\exp}\in O(\sum_{N=1}^{N_{\max}} \ee^{\lambda N})\in O(\ee^{\lambda N_{\rm max}})\in O(1/\epsilon)$}\DIFaddend .  The
time required saturates the Heisenberg limit, up to a multiplicative constant.
Specifically, $\lambda\approx 0.17$ for \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE}\DIFaddend .

\DIFdelbegin %DIFDELCMD < { %%%
\DIFdelend \fig{ITPEcmp} compares \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE }\DIFaddend to the information-theory PE (ITPE) method of Svore~\etal.  \DIFdelbegin %DIFDELCMD < }  %%%
\DIFdelend Although ITPE is inefficient and is non--adaptive, it is exact and so it is a natural benchmark to compare \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE }\DIFaddend against.  ITPE requires nearly five times the applications of $U$ to estimate $\phi$ if the eigenphase is of the form $\phi=2\pi k/t$ for integer $k<t$ and $t=10~000$.  
...
probability whereas \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
...
%DIF < We propose the following simple algorithm for phase estimation:
\DIFdelbegin %DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \label{eq:pgh2}
%DIFDELCMD < %%%
...
%DIF < An exponential distribution is chosen rather than a
%DIF < Gaussian because the exponentially shrinking probability more closely matches
%DIF < the exponentially decaying signal expected from experiments with duration much
%DIF < greater than $T_2$.  Randomization in this form is useful because it helps
%DIF < fight the algorithm becoming stuck in cases where the posterior variance
%DIF < underestimates the error.   
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < The same methodology applies when
%DIF < $T_2$ is unknown~\cite{granade_robust_2012}.  The only modification needed is that the $T_2$
%DIF <  in~\eq{pgh2} should be drawn from the prior
%DIF < $P(\phi,T_2)$.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < 
%DIF < There are several issues that need to be considered in order to make this approach practical.  The first issue that arises in these cases is that since we cannot increase the evolution time without limit, the likelihood function begins to flatten.  This creates an issue because the statistical fluctuations in the calculation in $\mu$ and $\sigma$ can easily be equivalent to the effect of the Bayesian update unless $m$ is large.  In particular, the update process becomes asymptotically unstable if $m \in \Omega(1/\delta^2)$ where $\delta$ is the maximum size of fluctuations of $P(E|\phi)$.  This can be understood by noting that the error in the mean of the accepted samples is $\Theta(1/\sqrt{m})$.  The reduction in the uncertainty in the posterior distribution after an experiment is proportional to $\delta$ if the prior distribution is Gaussian.  Thus if $m\in \Omega(1/\delta^2)$ then the sample noise in $\mu$ is greater than the reduction in $\sigma$, leading to instability.  Either $m$ must be increased or several experiments must be batched together to increase $\delta$  to address these issues.  This argument is made rigorous in the appendix.
%DIF < 
%DIF < The second issue is that, without modification, the probability that a given hypothesis survives $p$ rounds of rejection sampling step shrinks exponentially with $p$.  This can be combatted by taking
%DIF < \begin{equation}
%DIF < \kappa_E = \frac{1+e^{-M/T_2}}{2},
%DIF < \end{equation}
%DIF < which makes the probability of accepting a sample $1$ if $\theta = \phi_{\rm true}$ even when $T_2\not\gg M$. 
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \fig{T2plot} shows that \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
...
\DIFaddend \fig{T2plot} shows the performance of \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE }\DIFaddend when the initial quantum state is an eigenstate and is discarded after each experiment.  Performing phase estimation in this way minimizes the number of experiments, but can be prohibitively expensive if preparation of the initial eigenstate is prohibitively costly.  In such cases, it makes sense to follow the standard prescription for phase estimation by keeping the quantum state until it is clear that the initial eigenstate has been depolarized.  These depolarizations can cause \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE }\DIFaddend to become confused because the new data that comes in is only consistent with hypotheses that have been ruled out.  We address this by performing inexpensive experiments to assess whether the state has depolarized and then restart the learning process. Restarting can even be valuable when $T_2=0$ if the input state is a superposition of eigenvectors or to recover if the \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{RFPE }\DIFaddend becomes stuck (see \DIFdelbegin \DIFdel{appendix}\DIFdelend \DIFaddbegin \app{var-reduction}\DIFaddend ).
...
\end{equation}
If $\tau=0.1$\DIFaddbegin \DIFadd{, }\DIFaddend the probability of measuring $0$ is approximately $0.998$ and hence measuring $1$ implies that the hypothesis that the prior is correct can be rejected at $p \le 0.002$. \DIFdelbegin \DIFdel{The Bayes factor to determine when to reset as discussed }\DIFdelend \DIFaddbegin \DIFadd{A Bayesian analysis of our reset rule
is given }\DIFaddend in \app{bf}.

If the test fails at a fixed level of significance then Step 3 restarts the learning process. It does not, however, make sense to throw away all the spectral information that has been learned when the algorithm is restarted.  Step 3 reflects this by checking to see if the current estimate of the eigenphase corresponds to a known eigenstate and then sets $\mu$ to be the estimate of the eigenvalue and $\sigma$ to be its uncertainty if such an identification is made.  This allows the algorithm to resume learning once the depolarized state is projected onto a known eigenstate.
...
We have provided a new approach to phase estimation that is not only requires
less experimental time than conventional PE proposals\DIFaddbegin \DIFadd{, }\DIFaddend but also is resilient
to experimental noise.  The simplicity of the algorithm further means that it
can be implemented \DIFaddbegin \DIFadd{on a classical host }\DIFaddend in a few hours\DIFdelbegin \DIFdel{but also that it can be executed on an embedded controller or }\DIFdelend \DIFaddbegin \DIFadd{, or can be embedded onto
}\DIFaddend an FPGA that is dedicated to controlling a quantum system. Since \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
\DIFdel{runs
well }\DIFdelend \DIFaddbegin \DIFadd{RFPE runs
well even }\DIFaddend in the presence of noise and imperfections in the system, it may
allow more sophisticated quantum simulations to be run without raising the
specter of decoherence ruining the experiment. \DIFaddbegin \DIFadd{This is especially critical in
lieu of fault-tolerance, as current experiments push past the classical
regime.
}\DIFaddend 

Looking forward, our work \DIFdelbegin \DIFdel{illustrates that phase estimation has not been solved in its entirety.  Rather}\DIFdelend \DIFaddbegin \DIFadd{shows how phase estimation can be made more
experimentally relevant.  Indeed}\DIFaddend , the problem of how to best infer an unknown
eigenvalue in the presence of experimental constraints is, and will likely
remain, an important problem for the foreseeable future. The flexibility,
efficiency and robustness of approximate Bayesian inference will no doubt make
it an indispensable tool for facing such challenges.
\DIFaddbegin 
...
An important drawback of \DIFdelbegin %DIFDELCMD < { %%%
\DIFdel{RFPE }%DIFDELCMD < } %%%
...
%=============================================================================
\section{Stability of Rejection \DIFdelbegin %DIFDELCMD < { %%%
\DIFdelend Filtering \DIFdelbegin %DIFDELCMD < } %%%
\DIFdelend PE}
...
alternative, one may prefer to use the Bayes factor \DIFaddbegin \DIFadd{to test whether the rejection
...
been lost, as described in the main body}\DIFaddend . For a uniform prior over
