...
and statistical inference in high--dimensional problems~\cite{doucet2000sequential,del2012adaptive,van2000unscented,liu2001combined}.  While these methods work well in conventional settings, 
...
and the computer used to control its dynamics.  The latency can be reduced to acceptable levels by allowing the inference to performed inside
...
system.  These restrictions can preclude the use of traditional particle filter methods.
...
array of small processing cores. Thus \CRej~is well suited for inference
 using hybrid computing platforms based on GPU and FPGA computing and
 memory restricted computing environments.
...
We start in \sec{smc} by discussing rejection sampling and particle filter methods. We
...
The second problem is that if $\sum_x P(E|x)P(x)$ is exponentially small then rejection sampling will not be efficient.  Given that the dimension of the state
...
by using idea from particle filter methods and using approximate, rather than exact, rejection sampling.
