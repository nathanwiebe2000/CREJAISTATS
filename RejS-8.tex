...
bad samples $\{x : \kappa_E < \Pr(E | x)\}$. This bound holds for one update,
such that it is useful to also consider the effect on an entire set of
experiments. Here, we show the effects of choosing an inappropriate $\kappa_E$
for the frequency estimation model of \eq{inv-model}. In particular,
we use the particle guess heuristic to design 100 measurements, then update using
...
of $\kappa_E$. Importantly, since the PGH tends to result in likelihoods with
peaks of order unity, any $\kappa_E < 1$ will generate bad samples in this case.
