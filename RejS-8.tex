...
reduces the memory required relative to conventional SMC methods.
We also provide an approximate form of rejection sampling that allows the algorithm to be used
in cases where traditional rejection sampling is not efficient.
Finally, we consider several numerical examples that illustrate that ideas will work well in memory restricted
problems that can arise in embedded applications.
\end{abstract}
...
\label{sec:intro}
%=====================================================================
Particle filter methods have in recent years become an indispensable tool for model selection, object tracking
and statistical inference in high--dimensional problems.  While these methods work well in conventional settings, 
an interesting question to ask is whether particle filter methods can be used in a streaming setting wherein the
user has access only to a constant amount of memory.  These problems are more than just curiousities.  If we
consider tracking the motion of rapidly moving objects, for example, then it is common that the dynamics of
the system can radically change over the time required to communicate between a system
and a computer used to control its dynamics.  Such problems could be addressed by allowing the inference to performed inside
the device itself, but this often places prohibitive restrictions on the processing power and memory of the 
device.  Such restrictions can preclude the use of existing machine learning methods in such control problems.

We present an approach that we call rejection filtering that employs rejection sampling to produce posterior samples in a
\emph{streaming} fashion. Because our algorithm does not need to record more
than a constant number of samples at any one time, \CRej~uses significantly
less memory than existing particle filter methods. Moreover, we show that our algorithm can be
easily parallelized at a fine-grained level, such that it can be used with an
array of small processing cores. Thus, \CRej~allows for finding posterior
samples using hybrid computing platforms based on GPU and FPGA computing, and
for embedding Bayesian inference in experimental apparatus.

Our algorithm accomplishes these gains by using particle filtering resampling
and variational methods in a rejection sampling context. By using streaming
algorithms for computing summary statistics and principal components, \CRej~
can employ these methods to boost rejection sampling acceptance rates without
compromising the online and parallel nature of rejection sampling. We also
show that \CRej~can be applied to model selection problems and to time-
dependent and state-space models by using diffusive updates similar to
computer vision applications.

We start in \sec{smc} by reviewing existing particle filtering methods. We
then detail the \CRej~algorithm and discuss applications to state-space models
...
computing Bayesian mean estimators using \CRej. In \sec{variational}, we
discuss the use of variational inference as a replacement for instrumental
distributions in the resampling step. Finally, in \sec{numerical-experiments},
we show numerical examples of the performance of \CRej~for periodic and
strongly-peaked test cases.
...
The Liu-West resampling algorithm [cite] interpolates between these two
...
approximation. [Cite SMC-ABC using summary statistics \cite{sisson_sequential_2007} over \emph{data}, not
samples, to do similar in likelihood-free context.] This observation is key to
...
%        and a mapping $\operatorname{F}$ between $(\mu,\sigma^2)$ to a family of probability distributions that has mean $\mu$ and covariance matrix $\Sigma$. Typically $F$ will yield a Gaussian distribution but other possibilities exist.
        \Function{\CRej Update}{$\vec{E}$, $\mu$, $\Sigma$, $m$, $\kappa_E$, $r$}
...
%---------------------------------------------------------------------
The idea of rejection filtering is to avoid storing the entirety of a
particle cloud used to represent the prior and instead only store
 summary statistics that are incrementally
computed from samples drawn from the prior.  This is made possible
using rejection sampling.  Specifically our process works as follows.
Begin by drawing a sample from $P(x)$.  Then reject that sample with
...
Thus rejection sampling can be used to perform Bayesian inference.  We
call this approach rejection filtering.
...
%described above, as it allows a generic family of distributions specified by the mean
%and covariance matrix of the sample data output by the rejection sampling step.  We use incremental
%algorithms to compute both the mean and the covariance matrix for the samples that are accepted
%by the rejection sampling step.  This is done to keep the memory usage low, which further exemplifies
%the memory efficient nature of our algorithm.  Generalization of this model to Gaussian mixture models
%is straightforward. \textbf{CG: if we have space, may be interesting to expand. Summary
%statistics here require associating newly accepted particles with the appropriate accumulation
%registers, then incrementing the correct $N_a$. That mixture distributions can also be
%\emph{efficiently} estimated is something you don't even get with LW SMC, mLW not withstanding.}
...
We can herald such failures in \CRej~by tracking the total number
of accepted particles $N_a$ in each update, as $N_a$ estimates
$P(E) = \sum_x P(E | x) P(x)$.
Alternatively, we can to better by
...
allows \CRej~to track stochastic processes in a similar way as \CSMC, as is illustrated
in \fig{crej-diffusion}.

Formally, we produce consider our posterior distribution to be found by
\begin{equation}
\tilde{P}(x|E;t) = P(x|E;t) \star \mathcal{B}(0,\beta),
\end{equation}
where $\mathcal{B}$ is a distribution with zero mean and standard deviation
$\beta$ and $\star$ denotes a convolution over $x$.
Convolution is in general an expensive operation, but since \CRej~uses a
...
$\tilde{P}(x|E;t)$ is $s+\beta$ and the mean remains $\bar{x}$.
...
Consider running \CRej~in parallel for two
...
estimate of the Bayes factor $K$,
\begin{equation}
    K := \frac{\prod_i P(E_i | A)}{\prod_i P(E_i | B)} = \frac{\expect{\prod_i P(E_i | x, A)}{x}}{\expect{\prod_i P(E_i | x, B)}{x}}.
\end{equation}
(Cite Akaike here.) If $K > 1$, then model $A$ is to be preferred as an explanation
...
divergences for when $N_a = 0$. Since this accumulation procedure
estimates the total likelihood from a two-outcome event
(acceptance/rejection of a sample), the value of $\beta = 1 / 2$ works
well to deal with the zero-likelihood case (cite CF+RBK).
...
Here, we demonstrate the effectiveness of our \CRej~algorithm using as an
...
multimodality in the posteriors. Thus, \CRej~succeeds in these cases only
...
%\begin{equation}
%    \label{eq:inversion-precession-nogamma}
%    \Pr(1 | x; t, x_-) = \cos^2((x - x_-) t / 2).
%\end{equation}
%
%
%%
%%\begin{algorithm}
%%    \caption{Particle Guess Heuristic (PGH) for \CRej}
%%    \label{alg:pgh}
%%    \begin{algorithmic}
%%        \Require Prior \CRej~state $\mu$, $\Sigma$. Resampling kernel $\operatorname{F}$.
%%        \Ensure  An experiment $(t, x_-)$.
%%        \Function{$\text{PGH}_\text{\CRej}$}{$\mu$, $\Sigma$}
%%            \State $x_- \sim \operatorname{F}(\mu, \Sigma)$
%%            \State $t \gets 1 / \sqrt{{\rm Tr}(\Sigma)}$
%%            \State \Return $(t, x_-)$.
%%        \EndFunction
%%    \end{algorithmic}
%%\end{algorithm}
%
%In order to understand the possible failure modalities of \CRej, it is also useful to
%consider more tightly peaked periodic likelihoods. We can form such test cases
%by simply raising our original likelihoods to a power $\gamma$,
%\begin{equation}
%    \label{eq:inversion-likelihood}
%    \Pr(1 | \omega; t, \omega_-) = \cos^{2\gamma} ([\omega - \omega_-] t / 2).
%\end{equation}
%Similarly peaked likelihoods arise in the use of maximum \emph{a posteri} estimators
%[cite], and emulate the low acceptance rate of batched updates such as discussed in
%\app{batched-updates}.
%
%%Using \CRej~together with the PGH to learn $x$ with the $\gamma = 5$ model,
%%we obtain the Bayes risk shown in \fig{crej-risk-vs-m} after 100 updates, and
%%with a varying number of sampling attempts $m$.
%%For each of several fixed values of $m$, we show the scaling of \CRej~with
%%the number of Bayes updates and the computation effort in \fig{crej-risk-vs-n}.
%%From these demonstrations, we see that $m = 300$ suffices to continue learning
%%for large numbers of updates, and offers a good tradeoff between the amount of
%%evidence and computational effort required.
%
%A similar problem that occurs frequently in electrical engineering and experimental
%physics is that there is an unknown frequency for a device that drifts with time.
...
\begin{figure}
    ...
    \caption{
        ...
        \CRej~with diffusion in the true model according to
...
We design experiments $(x_-, t)$ using a variant of the particle guess
heuristic (PGH) \cite{wiebe_hamiltonian_2014}. This
heuristic serves as a useful example of the use of prior information in a
heuristically-adaptive context.  The heuristic picks $x_-$ to be a random
point sampled from the prior and $t=1/\sqrt{{\rm Tr}~ \Sigma}$.  We use
it because it is known
...
Thus, \CRej~algorithm continues to be useful in the case of time-dependent
...
Bayesian inference to classify an unknown digit taken from the MNIST repository in
one of two classes.  Here we consider two cases: $1$ vs $0$ and even vs odd.

We cast the problem in the language of Bayesian inference by assuming the following likelihood
function, which predicts the probability that a given pixel takes the value $E$ given that training
image $x$ is the true model that it drew from.
\begin{equation}
...
posterior probability distriution, given a uniform initial distribution, will typically be sharply peaked around
...
Bottom plots are the corresponding plots for the odd/even digits.}
\end{figure}
...
We perform these experiments using an adaptive guess heuristic, similar to the PGH employed earlier.
...
\begin{figure}
\includegraphics[width=0.49\columnwidth]{ErrorPlot.pdf}
\caption{Classification errors for odd/even MNIST digits for rejection filtering with 784 maximum experiments distributed over 1,3,5 restarts (from left to right).}
\end{figure}
...
%        and a mapping $\operatorname{F}$ between $(\mu,\sigma^2)$ to a family of probability distributions that has mean $\mu$ and covariance matrix $\Sigma$. Typically $F$ will yield a Gaussian distribution but other possibilities exist.
        ...
        \Function{\CRej BatchUpdate}{$\vec{E}$, $m$, $\kappa_E$, $\mu$, $\Sigma$, $r$, $N_{\rm batch}$}
  \State{$(M,S,N_a) \gets 0$}
...
\textbf{CG: this should still be somewhere.}
This approach is
reminiscent of assumed density filtering~\cite{minka_expectation_2001}, which uses a similar strategy
for modeling the prior distribution but is less memory efficient than
our method.
