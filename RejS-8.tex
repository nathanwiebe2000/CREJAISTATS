...
the device itself \cite{lavalle_sensor_2013}, but this often places prohibitive restrictions on the processing power and memory on processors embedded on or near the system.
These restrictions can preclude the use of traditional particle filter methods.

We present an approach that we call \emph{rejection filtering} that efficiently samples from
...
wherein samples correspond to experiments that can be used to optimize the inference procedure.
In particular, our approach uses well-motivated experiment design heuristics in conjunction with rejection
filtering.
This amounts to a form of selective sampling that is computationally inexpensive, easy to parallelize and deploy and can operate in a much more memory restricted environment than existing approaches~\cite{sivaraman2010general,kapoor2007active}.
...
We consider an algorithm to be efficient if it runs in $O(\operatorname{dim}(x)))$ time.
...
We discuss this batched version of the
inference algorithm in~\alg{batchcrej} (Supplemental Material).
...
We overcome such instabilities through by invoking random restarts, as outlined in the next section,
...
Close proximity is necessary in many applications (such as in control problems) to make the latency
...
one of two classes.  Here we consider two cases: the restricted case of classifying only $1$ digits and $0$ digits and classifying digits as even or odd.
...
\caption{(top left) Heat map of frequency pixel is queried in 1 vs 0 data set.  (top right) Heat map of variance of pixel data over data set.
Bottom plots are the corresponding plots for the odd vs even data set.}\label{fig:HM}
\end{figure}
