...
and statistical inference in high--dimensional problems~\cite{doucet2000sequential,del2012adaptive,van2000unscented}.  While these methods work well in conventional settings, 
an interesting question to ask is whether particle filter methods can be used in a streaming setting wherein the
user has access only to a constant amount of memory.  Memory restricted problems are more than just curiousities.  If we
...
device.  These restrictions can preclude the use of particle filter methods in such control problems.

We present an approach that we call rejection filtering that employs rejection sampling and resampling to produce posterior samples in a
\emph{streaming} fashion. This allows rejection filtering does to perform the inference task while storing no more
than a constant number of samples at any one time in typical use cases.  Rejection filtering therefore uses significantly
...
samples using hybrid computing platforms based on GPU and FPGA computing, as
well as memory restricted computing environments.
For example, \CRej~allows for inference to be embedded in novel contexts such
as very small FPGAs for low-temperature use \cite{hornibrook_cryogenic_2015} or for
...
In many applications, we want to maintain both the advantages of
particle filtering and of rejection sampling. We introduce rejection
filtering as a method which uses resampling to connect these two
algorithms. 

Specifically, our process works as follows.
...
Thus, rejection filtering can be used to perform Bayesian inference.
