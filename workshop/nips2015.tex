...
We present an approach that we call \emph{rejection filtering} (RF), which samples from
an approximation to the posterior by using rejection sampling and resampling in concert.
This allows it approximately update the prior distribution while considering no more
than one sample at a time in typical use cases.
...
For simplicity we take all variables to be discrete in the following, however, the generalization to the continuous
case is trivial.
...
Thus the accepted samples are distributed identically to those drawn from the posterior distribution.

The acceptance probability
can be increased by the following rescaling $P(x|E) \rightarrow P(x|E)/\kappa_E$ where
...
It does, however, boost the probability of acceptance to $P(E)/\kappa_E$, which
can dramatically improve performance if $\max_x P(E|x) \ll 1$.
...
by combining rejection sampling with ideas from  \emph{sequential Monte Carlo} methods~\cite{doucet2000sequential,del2012adaptive,van2000unscented,liu2001combined} to allow approximate inference to be performed while retaining only summary statistics of the accepted samples.
...
The samples used in the next round of rejection filtering are then drawn from this new
distribution; we dub this process resampling because of its similarity to resampling in sequential Monte Carlo methods~\cite{doucet2000sequential,del2012adaptive,van2000unscented,liu2001combined}.

To clarify how resampling is used in practice in rejection filtering, let us consider the case of a Gaussian model for the posterior.  This model is uniquely specified by computing the mean and the covariance matrix for the samples
...
The rejection filtering algorithm is described in detail in~\alg{crej} for the case of a prior model that depends only on the first two moments of the accepted samples. We discuss the efficiency of the algorithm in the following theorem.
...
In \app{batched-updates}, we give a version of our algorithm that treads width for depth by batching the data
and evaluating particles in parallel.
...
This leads to an efficient method for approximate sampling from the posterior distribution assuming that $\delta$ is constant and $P(E)/\kappa_E$ is at most polynomially small.  
...
This application is of central importance in quantum physics experiments wherein the precise estimation of such frequencies involves substantial
...
$
where $\phi(t_k)$ is the unknown frequency evaluated at time $t_k$ and $M$ and $\theta$ are experimental settings that are chosen to yield a near--optimal
experiment given the prior distribution.  We take $\phi(t_k)$ to follow a normal random walk in $k$ with standard deviation $\pi / 120$.
...
Concretely, we consider an inference problem with a single observed variable $E\in\{0, 1\}$, decision variables $x_-$ and $t$ and with a single hidden variable $x$ that we allow
to vary with time.
Our aim is to infer the current value of $x$ given values of $E$ obtained for different values of $(x_-,t)$.
The likelihood of measuring $E=1$ for these experiments is
\begin{equation}
  \label{eq:inv-model}
...
Scaling of the classification error as a function of the number of pixels considered in MNIST digit classification.
    }
\end{figure}
